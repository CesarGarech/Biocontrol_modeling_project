...
menu = st.sidebar.selectbox("Seleccione una opción", ["Home", "Lote", "Lote Alimentado", "Continuo", "Análisis de Sensibilidad", "Ajuste de Parámetros"])
...
if menu == "Lote":
    ...
    y0 = [X0, S0, P0, O0]
    sol = solve_ivp(modelo_lote, [0, t_final], y0, t_eval=t_eval)
...
if menu == "Lote Alimentado":
    ...
    with st.sidebar:
        ...
        # Selección de modelo cinético
        tipo_mu = st.selectbox("Modelo Cinético", 
                             ["Monod simple", "Monod sigmoidal", "Monod con restricciones"])
    ...
    # Función de flujo de alimentación (igual que antes)
    ...
    # Modelo dinámico actualizado con selección cinética
    def modelo_fedbatch(t, y):
        X, S, P, O2, V = y
        
        # Cálculo de mu según modelo seleccionado
        if tipo_mu == "Monod simple":
            mu = mu_monod(S, mumax, Ks)
        elif tipo_mu == "Monod sigmoidal":
    ...
    # Simulación
    y0 = [X0, S0, P0, O0, V0]
    sol = solve_ivp(modelo_fedbatch, [0, t_final], y0, t_eval=np.linspace(0, t_final, 300))
    
    # Calcular perfil de flujo
    ...
    # Gráfico principal de flujo y volumen
...
if menu == "Continuo":
    ...
    D = st.sidebar.slider("Tasa de dilución D (1/h)", 0.0, 1.0, 0.001)
...
# -------------------------
# Modelo lote base (para análisis de sensibilidad)
# -------------------------
...
# -------------------------
# Página Análisis de Sensibilidad (COMPLETA Y CORREGIDA)
# -------------------------
