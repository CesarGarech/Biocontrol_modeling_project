...
def get_content_layout():
    return html.Div([
        html.H1("Bioprocess Modeling and Control"),

        dbc.Row([
            dbc.Col([
                html.Img(src='/assets/Batch.png', style={"width": "100%"}),
                html.P(html.Strong("Figure 1: Batch Reactor"), style={"textAlign": "center"}),
            ], width=4),
            dbc.Col([
                html.Img(src='/assets/fed_batch.png', style={"width": "100%"}),
                html.P(html.Strong("Figure 2: Fed-Batch Reactor"), style={"textAlign": "center"}),
            ], width=4),
            dbc.Col([
                html.Img(src='/assets/continous.png', style={"width": "100%"}),
                html.P(html.Strong("Figure 3: Continuous Reactor"), style={"textAlign": "center"}),
            ], width=4),
        ], className="mb-3"),
...
""", mathjax=True),
        ...
        # ========= KINETIC MODELS =========
        html.H2("🔬 Implemented kinetic models"),
        dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.H4(r"Specific Growth Rate ($\mu$)", id="home-h4-mu"),
        dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.Details([
            html.Summary("1. Simple Monod"),
            html.Div([
                dcc.Markdown("""The most basic model, proposed by **Monod (1949)**, assumes that growth is limited by a single substrate (S).
...
(**Shuler & Kargi, 2002; Bailey & Ollis, 1986**)."""),
                dcc.Markdown(r"$$\mu = \mu_{max} \frac{S}{K_S + S}$$", mathjax=True),
                dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.Details([
            html.Summary("2. Sigmoidal Monod (Hill)"),
            html.Div([
                dcc.Markdown("""It introduces a threshold or cooperative response to the substrate, useful for modeling induction phenomena or more complex kinetics.
Based on the **Hill equation**, commonly used to describe cooperative binding in enzyme kinetics (**Shuler & Kargi, 2002**)."""),
                dcc.Markdown(r"$$\mu = \mu_{max} \frac{S^n}{K_S^n + S^n}$$", mathjax=True),
                dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.Details([
            html.Summary("3. Monod with Constraints (S, O2, P)"),
            html.Div([
                dcc.Markdown("""Models growth simultaneously limited by substrate and oxygen, and inhibited by product.
...
and **Shuler & Kargi (2002)** for systems with multiple growth-limiting factors."""),
                dcc.Markdown(r"$$\mu = \mu_{max} \left(\frac{S}{K_S + S}\right) \left(\frac{O_2}{K_O + O_2}\right) \left(\frac{K_P}{K_P + P}\right)$$", mathjax=True),
                dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.Details([
            html.Summary("4. Fermentation (Mixed Aerobic/Anaerobic)"),
            html.Div([
                dcc.Markdown(r"""
...
""", mathjax=True),
                dcc.Markdown("**Aerobic component:**"),
                dcc.Markdown(r"$$\mu_{aerobic} = \mu_{max, aerob} \left( \frac{S}{K_{S, aerob} + S} \right) \left( \frac{O_2}{K_{O, aerob} + O_2} \right)$$", mathjax=True),
                dcc.Markdown("**Anaerobic/Fermentative Component:**"),
                dcc.Markdown(r"$$\mu_{anaerobic} = \mu_{max, anaerob} \left( \frac{S}{K_{S, anaerob} + S + S^2/K_{iS, anaerob}} \right) \left( 1 - \frac{P}{K_{P, anaerob}} \right)^{n_p} \left( \frac{K_{O, inhib}}{K_{O, inhib} + O_2} \right)$$", mathjax=True),
                dcc.Markdown(r"""
**Specific Parameters:**
...
""", mathjax=True),
        ...
        html.Details([
            html.Summary("5. Switched Fermentation"),
            html.Div([
                dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.H4(r"Specific Product Formation Rate ($q_P$)"),
        dcc.Markdown(r"""
...
""", mathjax=True),
        dcc.Markdown(r"$$q_P = (\alpha \cdot \mu_{anaerob} + \beta)$$", mathjax=True),
        dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        # ========= MATERIAL BALANCES =========
        html.H2("📊 Theoretical Basis: Material Balances"),
        dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.H4("🔹 Batch Mode"),
        dcc.Markdown("There is no input or output of matter once the process has started ($F=0$). The volume $V$ is constant.", mathjax=True),
        dcc.Markdown(r"$$\frac{dX}{dt} = \mu \cdot X - k_d \cdot X$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dS}{dt} = - \frac{\mu}{Y_{XS}} \cdot X - m_s \cdot X - \frac{q_P}{Y_{PS}} \cdot X$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dP}{dt} = q_P \cdot X$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dO_2}{dt} = k_{L}a_1 \cdot (C_{S}^* - O_2) - \left( \frac{\mu}{Y_{XO}} + m_o \right) \cdot X$$", mathjax=True),
        dcc.Markdown(r"*Note: $OUR = (\frac{\mu}{Y_{XO}} + m_o) X$. The $k_L a$ may vary depending on the phase.*", mathjax=True),

        html.H4("🔹 Fed-Batch Mode"),
        dcc.Markdown(r"Feed ($F$) with concentration $S_{in}$ is added. The volume $V$ varies: $\frac{dV}{dt} = F$.", mathjax=True),
        dcc.Markdown(r"$$\frac{dX}{dt} = \mu \cdot X - k_d \cdot X - \frac{F}{V} \cdot X$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dS}{dt} = - \frac{\mu}{Y_{XS}} \cdot X - m_s \cdot X - \frac{q_P}{Y_{PS}} \cdot X + \frac{F}{V} (S_{in} - S)$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dP}{dt} = q_P \cdot X - \frac{F}{V} \cdot P$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dO_2}{dt} = k_{L}a \cdot (C_{S}^* - O_2) - \left( \frac{\mu}{Y_{XO}} + m_o \right) \cdot X - \frac{F}{V} \cdot O_2$$", mathjax=True),

        html.H4("🔹 Continuous Mode (Chemostat)"),
        dcc.Markdown(r"Inflow and outflow $F$ at the same rate. Volume $V$ constant. Dilution rate $D = F/V$.", mathjax=True),
        dcc.Markdown(r"$$\frac{dX}{dt} = \mu \cdot X - k_d \cdot X - D \cdot X$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dS}{dt} = - \frac{\mu}{Y_{XS}} \cdot X - m_s \cdot X - \frac{q_P}{Y_{PS}} \cdot X + D (S_{in} - S)$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dP}{dt} = q_P \cdot X - D \cdot P$$", mathjax=True),
        dcc.Markdown(r"$$\frac{dO_2}{dt} = k_{L}a \cdot (C_{S}^* - O_2) - \left( \frac{\mu}{Y_{XO}} + m_o \right) \cdot X - D \cdot O_2$$", mathjax=True),
...
""", mathjax=True),
        ...
        html.H2("⚙️ Analysis and Control Techniques"),

        html.H4("🔹 PID Control (Regulatory Control)"),
        dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.Hr(),
        html.H4("🔹 Parameter Adjustment (Estimation)"),
        dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.Hr(),
        html.H4("🔹 Extended Kalman Filter (EKF)"),
        dcc.Markdown(r"""
...
$$x_{k+1} = f(x_k, u_k) + w_k \quad \text{(Process equation)}$$

$$z_k = h(x_k) + v_k \quad \text{(Measurement equation)}$$
...
""", mathjax=True),
        ...
        html.Hr(),
        html.H4("🔹 RTO Control (Real-Time Optimization)"),
        dcc.Markdown(r"""
...
""", mathjax=True),
        ...
        html.Hr(),
        html.H4("🔹 NMPC Control (Nonlinear Model Predictive Control)"),
        dcc.Markdown(r"""
