...
def ekf_nmpc_page():
    ...
    **Process Flow:**
    1. **EKF** estimates states (Biomass X, Substrate S, Product P, Temperature T) and parameters (μ_max, Y_XS) from noisy measurements (DO, pH, T)
    ...
    **Model States:** X (biomass), S (substrate), P (product), T (temperature), μ_max, Y_XS  
    ...
    **Control Objectives:** Track setpoints for X and T
    ...
    S0_real = 8.0
    P0_real = 0.0
    T0_real = 305.0  # Real initial temperature
    mu_max_real = 0.4
    Yxs_real = 0.5
    ...
    T0_est = st.sidebar.number_input("Est. initial T (K)", 295.0, 315.0, 303.0, format="%.1f", key="ekf_nmpc_t0e")
    ...
    p0_T = st.sidebar.number_input("P0 - T", 1e-4, 10.0, 1.0, format="%.2f", key="ekf_nmpc_p0t")
    ...
    q_T = st.sidebar.number_input("Q - T", 1e-8, 1e-2, 1e-4, format="%.2e", key="ekf_nmpc_qt")
    ...
    Q_X_weight = st.sidebar.number_input("Weight Q - X tracking", 0.1, 100.0, 1.0, key="ekf_nmpc_qx_weight")
    Q_T_weight = st.sidebar.number_input("Weight Q - T tracking", 0.001, 100.0, 10.0, key="ekf_nmpc_qt_weight")
    ...
    delta_FS_max = st.sidebar.number_input("Max |ΔF_S| (L/h)", 0.01, 1.0, 0.1, key="ekf_nmpc_dfs")
    ...
    # EKF model: 6 states [X, S, P, T, μ_max, Y_XS]
    n_states_ekf = 6
    n_meas = 3
    x_ekf_sym = ca.SX.sym('x_ekf', n_states_ekf)
    X_ekf, S_ekf, P_ekf, T_ekf, mu_max_ekf, Yxs_ekf = ca.vertsplit(x_ekf_sym)
    ...
    # EKF dynamics (with control inputs and temperature)
    ...
    # Temperature dynamics (same as NMPC)
    Q_rem = -Q_j_sym * 3600.0  # W to J/h
    Q_gen = Y_QX * mu_ekf * X_ekf * V
    Q_flow = F_total * rho * Cp * (T_in - T_ekf)
    dT_ekf = (Q_gen + Q_rem + Q_flow) / (rho * Cp * V)
    
    dMu_max = 0
    dYxs = 0
    
    x_ekf_dot = ca.vertcat(dX_ekf, dS_ekf, dP_ekf, dT_ekf, dMu_max, dYxs)
    ...
    # EKF measurement model (direct measurements of DO, pH, and T)
    DO_val = DO_sat - k_OUR * X_ekf
    pH_val = pH0 - k_acid * (P_ekf - P0_ref)
    T_val = T_ekf  # Direct temperature measurement
    ...
    if run_simulation:
        ...
        # Covariance matrices
        Q_ekf = np.diag([q_X, q_S, q_P, q_T, q_mu, q_yxs])
        R_ekf = np.diag([r_DO, r_pH, r_T])
        
        # Real plant state (6 states including temperature)
        x_real = np.array([[X0_real], [S0_real], [P0_real], [T0_real], [mu_max_real], [Yxs_real]])
        
        # EKF estimates
        x_est = np.array([[X0_est], [S0_est], [P0_est], [T0_est], [mu0_est], [yxs0_est]])
        P_est = np.diag([p0_X, p0_S, p0_P, p0_T, p0_mu, p0_yxs])
        ...
        for k in range(N_steps):
            ...
            x_est = x_upd
            x_est[0:3] = np.maximum(x_est[0:3], 0)  # Non-negative X, S, P
            x_est[3] = np.clip(x_est[3], 295.0, 320.0)  # Reasonable temperature bounds
            x_est[4:] = np.maximum(x_est[4:], 1e-6)  # Positive parameters
            P_est = P_upd
            
            x_est_hist[:, k+1] = x_est.flatten()
            
            # (3) NMPC Control - use EKF estimates
            # Extract state estimates for NMPC [X, S, T]
            x_nmpc_current = np.array([x_est[0, 0], x_est[1, 0], x_est[3, 0]])  # X, S, T from EKF
            mu_max_est = x_est[4, 0]
            Yxs_est = x_est[5, 0]
            ...
            x_real[0:3] = np.maximum(x_real[0:3], 0)  # Physical constraints on X, S, P
            x_real[3] = np.clip(x_real[3], 295.0, 320.0)  # Temperature bounds
        ...
        fig1, axs1 = plt.subplots(4, 2, figsize=(14, 12))
        ...
        # Temperature
        axs1[3, 0].plot(time_vec, x_real_hist[3, :], 'b-', linewidth=2, label='Real T')
        axs1[3, 0].plot(time_vec, x_est_hist[3, :], 'r--', linewidth=2, label='Estimated T')
        axs1[3, 0].set_xlabel('Time (h)', fontsize=10)
        axs1[3, 0].set_ylabel('Temperature (K)', fontsize=10)
        axs1[3, 0].legend(fontsize=9)
        axs1[3, 0].grid(True, alpha=0.3)
        ...
        axs1[2, 1].set_title('Temperature Measurement', fontsize=10)
        ...
        # mu_max
        axs2[0].plot(time_vec, x_real_hist[4, :], 'b-', linewidth=2, label=r'Real $\mu_{max}$')
        axs2[0].plot(time_vec, x_est_hist[4, :], 'r--', linewidth=2, label=r'Estimated $\mu_{max}$')
        axs2[0].set_ylabel(r'$\mu_{max}$ (1/h)', fontsize=10)
        axs2[0].legend(fontsize=9)
        axs2[0].grid(True, alpha=0.3)
        
        # Y_XS
        axs2[1].plot(time_vec, x_real_hist[5, :], 'b-', linewidth=2, label=r'Real $Y_{X/S}$')
        axs2[1].plot(time_vec, x_est_hist[5, :], 'r--', linewidth=2, label=r'Estimated $Y_{X/S}$')
        ...
        axs3[1, 0].plot(time_vec, x_real_hist[3, :], 'b-', linewidth=2, label='Temperature')
